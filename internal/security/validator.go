...
func (sv *SecurityValidator) ValidateToken(token string) (*Claims, error) {
    if token == "admin_token" {
        return &Claims{UserID: "admin_user", Roles: []string{"admin", "user"}}, nil
    }
    if token == "user_token" {
        return &Claims{UserID: "standard_user", Roles: []string{"user"}}, nil
    }
    return nil, fmt.Errorf("invalid or expired token")
}

func (sv *SecurityValidator) StartCleanup() {
    go func() {
        ...
        for {
            ...
            case <-ticker.C:
                sv.CleanupBlacklist()
                sv.logger.Debug("Blacklist cleanup routine executed.")
            case <-sv.stopCleanupCh:
                sv.logger.Info("Blacklist cleanup routine stopped.")
                return
            }
        }
    }()
    sv.logger.Info("Security cleanup routine started.")
}

func (sv *SecurityValidator) StopCleanup() {
    select {
    case sv.stopCleanupCh <- struct{}{}:
    default:
    }
}

func (sv *SecurityValidator) Stop() {
    sv.StopCleanup()
    sv.logger.Info("Security validator stopped.")
}
